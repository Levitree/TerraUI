          ...
          <TForm :schema="schema" :initial-values="validInitial" @submit="onSubmit">
            <template #default="{ getFieldProps }">
              <TFormField name="name" label="Name" required>
                <TInput
                  v-bind="getFieldProps('name') as unknown as Record<string, unknown>"
                  placeholder="Ada Lovelace"
                />
              </TFormField>
              <TFormField name="email" label="Email" required>
                <TInput
                  v-bind="getFieldProps('email') as unknown as Record<string, unknown>"
                  type="email"
              ...
              <TFormField name="role" label="Role">
                <TSelect
                  v-bind="getFieldProps('role') as unknown as Record<string, unknown>"
                  :options="roleOptions"
                />
              </TFormField>
              <TFormField name="bio" label="Bio" hint="Max 200 chars">
                <TTextarea
                  v-bind="getFieldProps('bio') as unknown as Record<string, unknown>"
                  :rows="3"
                />
              </TFormField>
            </template>
